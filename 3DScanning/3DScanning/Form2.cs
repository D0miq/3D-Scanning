...
namespace _3DScanning
{
    ...
    public partial class Form : System.Windows.Forms.Form
    {
        ...
        public async void Reader_VisualiseDepth(object sender, MultiSourceFrameArrivedEventArgs e)
        {
            if (this.tabIndex == 1)
                    {
        ...
        public async void Reader_GenerateMesh(object sender, MultiSourceFrameArrivedEventArgs e)
        {
        ...
        public void DoWork(IProgress<int> progress)
        {
            ...
            {
                progress.Report(1);
                var meshBuilder = new Mesh.Builder();
                progress.Report(1);
                if (dispersionCB.Checked)
                {
                    this.depthData.FinalyzeDispersion();
                    this.meshBuilder.AddDispersion(this.depthData.Dispersion);
                }
                progress.Report(1);
                if (this.coloredMeshRB.Checked)
                {
                    this.meshBuilder.AddRealColors(this.depthData.Dispersion);
                } else if (this.scaledMeshRB.Checked)
                {
                    this.meshBuilder.AddComputedColors(this.colorData.Data);
                }
                progress.Report(1);
                this.meshBuilder.Build().GenerateMesh(path + "\\points" + counter + ".obj", mesh, false, true, "");
                progress.Report(1);
            });
        }
